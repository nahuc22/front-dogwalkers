...
export default function WalkerProfile() {
    ...
    // Función para obtener la imagen de portada correcta
    const getCoverImage = () => {
        if (isPet) {
            // Para perros: usar profileImage del pet o ícono default
            return petData?.profileImage || null;
        } else {
            // Para walkers: usar coverImage o profileImage o ícono default
            return walkerData?.coverImage || walkerData?.profileImage || null;
        }
    };

    // Estados editables - adaptados para walker o pet
    const [editedPrice, setEditedPrice] = useState(
        isPet ? '0' : (walkerData?.price || item?.price || '5')
    );
    const [editedDescription, setEditedDescription] = useState(
        isPet 
            ? (petData?.specifications || petData?.medicalCondition || 'Sin información adicional')
            : (walkerData?.description || 'Sin descripción profesional')
    );
    const [editedAge, setEditedAge] = useState(
        isPet 
            ? (petData?.age?.toString() || '3')
            : (walkerData?.age || item?.age || '27')
    );
    const [editedExperience, setEditedExperience] = useState(
        isPet 
            ? (petData?.breed || 'Raza desconocida')
            : (walkerData?.experience || item?.experience || '11 Months')
    );
    const [editedCoverImage, setEditedCoverImage] = useState(getCoverImage());
    ...
    useEffect(() => {
        ...
        const priceChanged = editedPrice !== (item?.walkerData?.price || item?.price || '5');
        const descChanged = editedDescription !== (item?.walkerData?.description || 'Sin descripción profesional');
        const ageChanged = editedAge !== (item?.walkerData?.age || item?.age || '27');
        const expChanged = editedExperience !== (item?.walkerData?.experience || item?.experience || '11 Months');
        const coverChanged = editedCoverImage !== (item?.walkerData?.coverImage || null);
        
        setHasChanges(priceChanged || descChanged || ageChanged || expChanged || coverChanged);
    }, [isEditing, editedPrice, editedDescription, editedAge, editedExperience, editedCoverImage, item]);
        ...
        try {
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                try {
                    ...
                    setEditedCoverImage(item?.walkerData?.coverImage || null);
                } finally {
            ...
            return (
                <View>
                    <View style={{ flexDirection: 'row', paddingVertical: scale(10) }}>
                        <View style={{ flex: 1 }}>
                            <Label text={isPet ? "Edad" : "Age"} style={{ fontSize: scale(13), color: appColors.gray }} />
                            {isEditing && isOwnProfile ? (
                                ...
                                <Label text={isPet ? `${editedAge} años` : `${editedAge} `} style={{ paddingVertical: scale(5), fontSize: scale(17) }} bold />
                            )}
                        </View>
                        <View style={{ marginLeft: scale(20), flex: 1 }}>
                            <Label text={isPet ? "Raza" : "Experience"} style={{ fontSize: scale(13), color: appColors.gray }} />
                            {isEditing && isOwnProfile ? (
                                <TextInput
                                    style={styles.smallInput}
                                    value={editedExperience}
                                    onChangeText={setEditedExperience}
                                    placeholder={isPet ? "Ej: Golden Retriever" : "Ej: 2 años"}
                                    placeholderTextColor="#999"
                                />
                            ) : (
                    ...
                    <View>
                        ...
                        {!isOwnProfile && (
                            <CustomButton 
                                label={isPet ? "Contactar Dueño" : "Agendar"} 
                                style={{ marginTop: scale(10) }} 
                            />
                        )}
                        {isPet && item.ownerName && (
    ...
    return (
        <SafeAreaView style={styles.container} edges={['bottom']}>
            {/* Botón de edición - solo visible si es el propio perfil del walker */}
            {isOwnProfile && (
                <EditButton
                    isEditing={isEditing}
                    ...
                    onPress={async () => {
                        if (isEditing) {
                            if (hasChanges) {
                                // Guardar cambios del perfil de walker
                                try {
                                    const profileData = {
                                        price: editedPrice,
                                ...
                                setEditedPrice(item?.walkerData?.price || item?.price || '5');
                                setEditedDescription(item?.walkerData?.description || 'Sin descripción profesional');
                                setEditedAge(item?.walkerData?.age || item?.age || '27');
                                setEditedExperience(item?.walkerData?.experience || item?.experience || '11 Months');
                                setEditedCoverImage(item?.walkerData?.coverImage || null);
                                setIsEditing(false);
                                setHasChanges(false);
                            }
            ...
            {editedCoverImage || item.img ? (
                ...
                >
                    ...
                    {/* Botón para cambiar foto de portada - solo visible cuando está editando */}
                    {isEditing && isOwnProfile && (
                        <TouchableOpacity 
                        ...
                        <View>
                            <View style={styles.header}>
                                ...
                                <View style={styles.cardsContainer}>
                                    ...
                                    <Card hideBorder={isPet} label={item.distance} subLabel={"km"}/>
                                    {!isPet && <Card label={item.rating} subLabel={"⭐"} />}
                                    {isPet ? (
                                        <Card subLabel={petData?.size || 'Mediano'} />
                                    ) : (
